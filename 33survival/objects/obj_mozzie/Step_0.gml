...
if instance_exists(oPlayer){
	...
	if collision_circle(x, y, 30, obj_enemy, false, true) {
		direction = scr_rotate(direction, point_direction(x, y, oPlayer.x, oPlayer.y), 1);
	}else{
		direction = scr_rotate(direction, point_direction(x, y, oPlayer.x + harass_x, oPlayer.y + harass_y), 2);
	}
	if speed > moveSpd { speed = moveSpd; }
	motion_add(direction, moveSpd);
	...
	if hp_jc != hp {
		...
		impact_alarm_1 --;
		if impact_alarm_1 <= 0 {sprite = spr_mozzie; sjdh = 0; tmd = 1;}
	}
}
